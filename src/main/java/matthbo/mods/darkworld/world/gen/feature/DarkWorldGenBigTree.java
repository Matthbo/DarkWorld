...
public class DarkWorldGenBigTree extends DarkWorldGenAbstractTree{
    ...
    public DarkWorldGenBigTree(boolean p_i2008_1_)
    {
        super(p_i2008_1_);
    ...
    void generateLeafNodeList()
    {
        ...
        this.field_175948_j.add(new DarkWorldGenBigTree.FoliageCoordinates(this.field_175947_m.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.func_175936_a(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.field_175948_j.add(new DarkWorldGenBigTree.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void func_180712_a(BlockPos pos, float par2, Block block)
    {
        int i = (int)((double)par2 + 0.618D);

        for (int j = -i; j <= i; ++j)
        {
            for (int k = -i; k <= i; ++k)
            {
                if (Math.pow((double)Math.abs(j) + 0.5D, 2.0D) + Math.pow((double)Math.abs(k) + 0.5D, 2.0D) <= (double)(par2 * par2))
                {
                    ...
                    {
                        this.func_175905_a(this.world, blockpos1, block, 0);
                    }
                }
            }
        }
    ...
    {
        if ((float)par1 < (float)this.heightLimit * 0.3F)
        {
            return -1.0F;
        }
        else
    ...
    void func_175940_a(BlockPos pos)
    {
        ...
        {
            this.func_180712_a(pos.up(i), this.leafSize(i), ModBlocks.darkLeaves);
        }
    }

    void func_175937_a(BlockPos pos, BlockPos pos1, Block block)
    {
        BlockPos blockpos2 = pos1.add(-pos.getX(), -pos.getY(), -pos.getZ());
        ...
        for (int j = 0; j <= i; ++j)
        {
            BlockPos blockpos3 = pos.add((double)(0.5F + (float)j * f), (double)(0.5F + (float)j * f1), (double)(0.5F + (float)j * f2));
            BlockLog.EnumAxis enumaxis = this.func_175938_b(pos, blockpos3);
            this.func_175903_a(this.world, blockpos3, block.getDefaultState().withProperty(BlockDarkLog.LOG_AXIS, enumaxis));
        }
    }
    ...
    void func_175941_b()
    {
        ...
        {
            DarkWorldGenBigTree.FoliageCoordinates foliagecoordinates = (DarkWorldGenBigTree.FoliageCoordinates)iterator.next();
